...
Flexible statistics and analytics plugin for Medusa with pluggable providers, composable charts, and alerting.

| Field | Details |
|-------|---------|
| **Status** | In Review |
| **Repository** | [github.com/tax1driver/medusa-stats](https://github.com/tax1driver/medusa-stats) |
| **Version** | <StarterVersion repo="tax1driver/medusa-stats" fallback="1.0.10" /> |
| **Author** | [tax1driver](https://github.com/tax1driver) |

## Features

- **Pluggable Statistics Providers** — Define and register custom providers for domain-specific metrics
- **Views and Chart Composition** — Organize metrics into configurable dashboards
- **Composite Statistics** — Build statistics that depend on other statistic outputs
- **Alerting System** — Configure threshold/comparative alerts with event emission
- **Admin Interface** — Manage views, options, and lifecycle operations from Medusa Admin
- **Caching Support** — Improve performance with cached statistic results

## Compatibility

- **Medusa Version**: `>= v2.11.2`
- **Node Version**: `>= 20`
...
Add the plugin and statistics module to your `medusa-config.ts`:

```typescript
modules: [
  {
    resolve: "medusa-stats/modules/statistics",
    dependencies: [ContainerRegistrationKeys.QUERY], // Query dependency is required for provider injection
    options: {
      providers: [
        {
...
Start with the built-in `common` provider, then add your own providers as needed. See [Providers](#providers) for examples.

## Overview

`medusa-stats` is a flexible statistics module for Medusa that supports custom metric definitions, chart-oriented organization, composable/stat-transform calculations, and configurable alert workflows.
